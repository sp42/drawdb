...
export function getJsonType(f): string {
...
export function getTypeString(field, dbms = "mysql", baseType = false) {
  if (dbms === "mysql") {
    ...
    if (isSized(field.type)) {
      const type = field.type === "BINARY" ? "bit"
        : field.type === "VARBINARY" ? "bit varying" : field.type.toLowerCase();
...
export function hasQuotes(type): boolean {
  return ["CHAR", "VARCHAR", "BINARY", "VARBINARY", "ENUM", "DATE", "TIME", "TIMESTAMP", "DATETIME"].includes(type);
}

export function parseDefault(field): string {
  if (strHasQuotes(field.default) || isFunction(field.default) || isKeyword(field.default) || !hasQuotes(field.type))
    return field.default;

  return `'${field.default}'`;
}

export function jsonToMySQL(obj): string {
  return `${obj.tables.map((table) =>
...
export function jsonToPostgreSQL(obj): string {
  return `${obj.types.map((type) => {
    const typeStatements = type.fields.filter((f) => f.type === "ENUM" || f.type === "SET")
      .map((f) => `CREATE TYPE "${f.name}_t" AS ENUM (${f.values
        .map((v) => `'${v}'`)
        .join(", ")});\n`,
      );
    if (typeStatements.length > 0)
      return (typeStatements.join("") + `${type.comment === "" ? "" : `/**\n${type.comment}\n*/\n`}CREATE TYPE ${type.name} AS (\n${type.fields.map((f) => `\t${f.name} ${getTypeString(f, "postgres")}`).join("\n")}\n);`);
    else
      return `${type.comment === "" ? "" : `/**\n${type.comment}\n*/\n`}CREATE TYPE ${type.name} AS (\n${type.fields.map((f) => `\t${f.name} ${getTypeString(f, "postgres")}`).join("\n")}\n);`;

  })}\n${obj.tables.map((table) => `${table.comment === "" ? "" : `/**\n${table.comment}\n*/\n`}${table.fields.filter((f) => f.type === "ENUM" || f.type === "SET")
    .length > 0
    ? `${table.fields.filter((f) => f.type === "ENUM" || f.type === "SET").map((f) => `CREATE TYPE "${f.name}_t" AS ENUM (${f.values.map((v) => `'${v}'`).join(", ")});\n\n`
    )}`
    : ""
    }CREATE TABLE "${table.name}" (\n${table.fields
      .map((field) =>
        `${field.comment === "" ? "" : `\t-- ${field.comment}\n`}\t"${field.name}" ${getTypeString(field, "postgres")}${field.notNull ? " NOT NULL" : ""}${field.default !== "" ? ` DEFAULT ${parseDefault(field)}` : ""
        }${field.check === "" || !hasCheck(field.type)
          ? ""
          : ` CHECK(${field.check})`
        }`,
...
export function getSQLiteType(field): string {
  switch (field.type) {
    case "INT":
    case "SMALLINT":
    case "BIGINT":
...
export function getInlineFK(table, obj): string {
  let fk = "";
...
export function jsonToSQLite(obj): string {
  return obj.tables
    .map((table) => {
...
export function jsonToMariaDB(obj): string {
  return `${obj.tables.map((table) => `${table.comment === "" ? "" : `/* ${table.comment} */\n`
...
export function jsonToSQLServer(obj): string {
  return `${obj.types.map((type) => {
    return `${type.comment === "" ? "" : `/**\n${type.comment}\n*/\n`
...
export function isSized(type: string): boolean {
  return ["CHAR", "VARCHAR", "BINARY", "VARBINARY", "TEXT"].includes(type);
}

export function hasPrecision(type: string): boolean {
  return ["DOUBLE", "NUMERIC", "DECIMAL", "FLOAT"].includes(type);
}

export function hasCheck(type: string): boolean {
  return ["INT", "SMALLINT", "BIGINT", "CHAR", "VARCHAR", "FLOAT", "DECIMAL", "DOUBLE", "NUMERIC", "REAL"].includes(type);
}

export function getSize(type: string): string | number {
  switch (type) {
    case "CHAR":
    case "BINARY":
      return 1;
