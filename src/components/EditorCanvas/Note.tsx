...
export default function Note({ data, onMouseDown }) {
  ...
  return (
    <g onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}    >
      <path d={`M${data.x + fold} ${data.y} L${data.x + w - r} ${data.y
        } A${r} ${r} 0 0 1 ${data.x + w} ${data.y + r} L${data.x + w} ${data.y + data.height - r
        } A${r} ${r} 0 0 1 ${data.x + w - r} ${data.y + data.height} L${data.x + r
        } ${data.y + data.height} A${r} ${r} 0 0 1 ${data.x} ${data.y + data.height - r
        } L${data.x} ${data.y + fold}`}
        fill={data.color} stroke={hovered ? "rgb(59 130 246)"
          : selectedElement.element === ObjectType.NOTE &&
            selectedElement.id === data.id ? "rgb(59 130 246)" : "rgb(168 162 158)"
        }
        strokeDasharray={hovered ? 4 : 0} strokeLinejoin="round" strokeWidth="1.2"
      />
      <path d={`M${data.x} ${data.y + fold} L${data.x + fold - r} ${data.y + fold
        } A${r} ${r} 0 0 0 ${data.x + fold} ${data.y + fold - r} L${data.x + fold
        } ${data.y} L${data.x} ${data.y + fold} Z`}
        fill={data.color}
        stroke={
          hovered ? "rgb(59 130 246)"
            : selectedElement.element === ObjectType.NOTE &&
      ...
      >
        <div className="text-gray-900 select-none w-full h-full cursor-move px-3 py-2">
          <div className="flex justify-between gap-1 w-full">
            ...
            {(hovered ||
              (selectedElement.element === ObjectType.NOTE &&
                ...
                !layout.sidebar)) && (
                <div>
                  <Popover visible={selectedElement.element === ObjectType.NOTE && selectedElement.id === data.id && selectedElement.open && !layout.sidebar}
                    onClickOutSide={() => {
                    ...
                    content={
                      <div className="popover-theme">
                        ...
                        <div className="w-[280px] flex items-center mb-2">
                          <Input value={data.title} placeholder={t("title")} className="me-2" onChange={(value) =>
                        ...
                        <div className="flex">
                          <Button icon={<IconDeleteStroked />} type="danger" block onClick={() => deleteNote(data.id, true)}                          >
                            {t("delete")}
                          </Button>
                        </div>
                      </div>
          ...
          <textarea id={`note_${data.id}`} value={data.content} onChange={handleChange} onFocus={(e) => setEditField({ content: e.target.value, height: data.height })}
            onBlur={handleBlur} className="w-full resize-none outline-none overflow-y-hidden border-none select-none" style={{ backgroundColor: data.color }}
          />
        </div>
      </foreignObject>
    </g>
