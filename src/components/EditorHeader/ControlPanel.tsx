...
export default function ControlPanel({ diagramId, setDiagramId, title, setTitle, lastSaved }: { diagramId: number, setDiagramId: (v: number) => {}, title: string, setTitle: (v: string) => {} }) {
  ...
  const [exportData, setExportData] = useState({ data: null, filename: `${title}_${new Date().toISOString()}`, extension: "" });
  ...
  const undo = () => {
    ...
    if (a.action === Action.ADD) {
      ...
      if (a.element === ObjectType.TABLE) {
        setRedoStack((prev) => [...prev, { ...a, x: tables[a.id].x, y: tables[a.id].y }]);
      ...
      if (a.element === ObjectType.AREA) {
        ...
        if (a.component === "field") {
          ...
          updateTable(a.tid, { fields: tables[a.tid].fields.filter((e) => e.id !== tables[a.tid].fields.length - 1).map((t, i) => ({ ...t, id: i })) });
        } else if (a.component === "index_add") {
          updateTable(a.tid, { indices: tables[a.tid].indices.filter((e) => e.id !== tables[a.tid].indices.length - 1).map((t, i) => ({ ...t, id: i })) });
        } else if (a.component === "index") {
          updateTable(a.tid, { indices: tables[a.tid].indices.map((index) => index.id === a.iid ? { ...index, ...a.undo } : index) });
          ...
          setTables((prev) =>
            prev.map((table) => {
              if (table.id === a.tid) {
                ...
                return { ...table, indices: temp.map((t, i) => ({ ...t, id: i })) };
              }
        ...
        setRelationships((prev) => prev.map((e, idx) => (idx === a.rid ? { ...e, ...a.undo } : e)));
      else if (a.element === ObjectType.TYPE) {
        if (a.component === "field_add")
          updateType(a.tid, { fields: types[a.tid].fields.filter((e, i) => i !== types[a.tid].fields.length - 1) });

        if (a.component === "field")
          updateType(a.tid, { fields: types[a.tid].fields.map((e, i) => i === a.fid ? { ...e, ...a.undo } : e) });
        else if (a.component === "field_delete")
      ...
      setTransform((prev) => ({ ...prev, pan: a.undo }));
      setRedoStack((prev) => [...prev, a]);
    }
  };
  ...
  const redo = () => {
    ...
    if (a.action === Action.ADD) {
      ...
      if (a.element === ObjectType.TABLE) {
        setUndoStack((prev) => [...prev, { ...a, x: tables[a.id].x, y: tables[a.id].y }]);
        updateTable(a.id, { x: a.x, y: a.y });
      } else if (a.element === ObjectType.AREA) {
        ...
        if (a.component === "field") {
          ...
          updateTable(a.tid, {
            fields: [
              ...tables[a.tid].fields,
              {
                name: "", type: "", default: "", check: "", primary: false, unique: false, notNull: false, increment: false, comment: "", id: tables[a.tid].fields.length,
              },
            ],
          });
        } else if (a.component === "index_add") {
          setTables((prev) =>
            prev.map((table) => {
              if (table.id === a.tid)
                return { ...table, indices: [...table.indices, { id: table.indices.length, name: `index_${table.indices.length}`, fields: [] }] };

              return table;
            }),
          );
        } else if (a.component === "index")
          updateTable(a.tid, { indices: tables[a.tid].indices.map((index) => index.id === a.iid ? { ...index, ...a.redo } : index) });
        else if (a.component === "index_delete")
          updateTable(a.tid, { indices: tables[a.tid].indices.filter((e) => e.id !== a.data.id).map((t, i) => ({ ...t, id: i })) });
        else if (a.component === "self")
          updateTable(a.tid, a.redo, false);
      } else if (a.element === ObjectType.RELATIONSHIP)
        setRelationships((prev) => prev.map((e, idx) => (idx === a.rid ? { ...e, ...a.redo } : e)));
      else if (a.element === ObjectType.TYPE) {
        if (a.component === "field_add")
          updateType(a.tid, { fields: [...types[a.tid].fields, { name: "", type: "" }] });
      } else if (a.component === "field")
        updateType(a.tid, { fields: types[a.tid].fields.map((e, i) => i === a.fid ? { ...e, ...a.redo } : e) });
      else if (a.component === "field_delete")
        updateType(a.tid, { fields: types[a.tid].fields.filter((field, i) => i !== a.fid) });
      else if (a.component === "self")
        updateType(a.tid, a.redo);

      setUndoStack((prev) => [...prev, a]);
    } else if (a.action === Action.PAN) {
      setTransform((prev) => ({ ...prev, pan: a.redo }));
      setUndoStack((prev) => [...prev, a]);
    }
  };
  ...
  const copyAsImage = () => toPng(document.getElementById("canvas") as HTMLElement).then(function (dataUrl) {
  ...
  const edit = () => {
    if (selectedElement.element === ObjectType.TABLE) {
      ...
      if (layout.sidebar) {
        ...
        setSelectedElement((prev) => ({ ...prev, open: true, editFromToolbar: true }));

    } else if (selectedElement.element === ObjectType.NOTE) {
      if (layout.sidebar) {
        setSelectedElement((prev) => ({ ...prev, currentTab: Tab.NOTES, open: false }));
  ...
  const duplicate = () => {
    switch (selectedElement.element) {
      case ObjectType.TABLE:
        addTable({ ...tables[selectedElement.id], x: tables[selectedElement.id].x + 20, y: tables[selectedElement.id].y + 20, id: tables.length });
        break;
      case ObjectType.NOTE:
        addNote({ ...notes[selectedElement.id], x: notes[selectedElement.id].x + 20, y: notes[selectedElement.id].y + 20, id: notes.length });
        break;
      case ObjectType.AREA:
        addArea({ ...areas[selectedElement.id], x: areas[selectedElement.id].x + 20, y: areas[selectedElement.id].y + 20, id: areas.length });
        break;
      default:
        break;
    }
  ...
  const menu = {
    file: {
      new: { function: () => setModal(MODAL.NEW) },
      new_window: {
        function: () => {
      ...
      open: { function: open, shortcut: "Ctrl+O" },
      save: { function: save, shortcut: "Ctrl+S" },
      save_as: { function: saveDiagramAs, shortcut: "Ctrl+Shift+S" },
      save_as_template: {
        function: () => {
          db.templates
            .add({ title: title, tables: tables, relationships: relationships, types: types, notes: notes, subjectAreas: areas, custom: 1 })
            .then(() =>
      ...
      delete_diagram: {
        warning: { title: t("delete_diagram"), message: t("are_you_sure_delete_diagram") },
        function: async () => {
          await db.diagrams.delete(diagramId).then(() => {
            setDiagramId(0);
      ...
      import_diagram: { function: fileImport, shortcut: "Ctrl+I" },
      import_from_source: {
      ...
      export_as: {
        children: [
          ...
          {
            JPEG: () => {
              toJpeg(document.getElementById("canvas") as HTMLElement, { quality: 0.95 }).then((dataUrl) => setExportData((prev) => ({ ...prev, data: dataUrl, extension: "jpeg" })));
              setModal(MODAL.IMG);
            },
          },
          {
            JSON: () => {
              setModal(MODAL.CODE);
              const result = JSON.stringify({ tables: tables, relationships: relationships, notes: notes, subjectAreas: areas, types: types, title: title }, null, 2);
          ...
          },
          {
            SVG: () => {
              ...
              toSvg(document.getElementById("canvas"), { filter: filter }).then((dataUrl) => setExportData((prev) => ({ ...prev, data: dataUrl, extension: "svg" })));
              setModal(MODAL.IMG);
            },
          },
          {
          ...
          },
          {
            DRAWDB: () => {
              const result = JSON.stringify(
                {
                  author: "Unnamed", title: title, date: new Date().toISOString(), tables: tables, relationships: relationships, notes: notes, subjectAreas: areas, types: types
                },
                null, 2
              );
        ...
        children: [
          {
            MySQL: () => {
              setModal(MODAL.CODE);
              const src = jsonToMySQL({ tables: tables, references: relationships, types: types });
          ...
          },
          {
            PostgreSQL: () => {
              setModal(MODAL.CODE);
              const src = jsonToPostgreSQL({ tables: tables, references: relationships, types: types });
              setExportData((prev) => ({ ...prev, data: src, extension: "sql" }));
            },
          },
          {
            SQLite: () => {
              setModal(MODAL.CODE);
              const src = jsonToSQLite({ tables: tables, references: relationships, types: types });
              setExportData((prev) => ({ ...prev, data: src, extension: "sql" }));
            },
          },
          {
            MariaDB: () => {
              setModal(MODAL.CODE);
              const src = jsonToMariaDB({ tables: tables, references: relationships, types: types });
              setExportData((prev) => ({ ...prev, data: src, extension: "sql" }));
            },
          },
          {
            MSSQL: () => {
              setModal(MODAL.CODE);
              const src = jsonToSQLServer({ tables: tables, references: relationships, types: types });
              setExportData((prev) => ({ ...prev, data: src, extension: "sql" }));
            },
          },
        ],
    ...
    view: {
      ...
      presentation_mode: {
        function: () => {
          setLayout((prev) => ({ ...prev, header: false, sidebar: false, toolbar: false }));
          enterFullscreen();
        },
      },
      field_details: {
      ...
      reset_view: { function: resetView, shortcut: "Ctrl+R" },
      show_grid: {
        state: settings.showGrid ? (<i className="bi bi-toggle-on" />) : (<i className="bi bi-toggle-off" />),
        function: viewGrid,
        shortcut: "Ctrl+Shift+G",
      },
      show_cardinality: {
        state: settings.showCardinality ? (<i className="bi bi-toggle-on" />) : (<i className="bi bi-toggle-off" />),
        function: () => setSettings((prev) => ({ ...prev, showCardinality: !prev.showCardinality }))
      },
      theme: {
        children: [
          {
    ...
    settings: {
      ...
      panning: {
        state: settings.panning ? (<i className="bi bi-toggle-on" />) : (<i className="bi bi-toggle-off" />),
        function: () => setSettings((prev) => ({ ...prev, panning: !prev.panning })),
      },
      table_width: { function: () => setModal(MODAL.TABLE_WIDTH) },
      language: { function: () => setModal(MODAL.LANGUAGE) },
      flush_storage: {
        warning: {
    ...
    return (
      <div className="py-1.5 px-5 flex justify-between items-center rounded-xl my-1 sm:mx-1 xl:mx-6 select-none overflow-hidden toolbar-theme">
        <div className="flex justify-start items-center">
          ...
          <Dropdown trigger="click" style={{ width: "240px" }} position="bottomLeft" render={
            <Dropdown.Menu>
              ...
              <Dropdown.Divider />
              <Dropdown.Item>
                <InputNumber field="zoom" label={t("zoom")} placeholder={t("zoom")} suffix={<div className="p-1">%</div>} onChange={(v) => setTransform((prev) => ({ ...prev, zoom: parseFloat(v) * 0.01 }))} />
              </Dropdown.Item>
            </Dropdown.Menu>
          }>
          ...
          <Tooltip content={t("add_table")} position="bottom">
            <button className="flex items-center py-1 px-2 hover-2 rounded" onClick={() => addTable()}><IconAddTable /></button>
          </Tooltip>
        ...
        <div className="flex justify-start items-center">
          ...
          <div className="ms-1 mt-1">
            <div className="flex items-center">
              <div className="text-xl ms-3 me-1" onMouseEnter={() => setShowEditName(true)} onMouseLeave={() => setShowEditName(false)} onClick={() => setModal(MODAL.RENAME)}>
            ...
            <div className="flex justify-between items-center">
              <div className="flex justify-start text-md select-none me-2">
                {Object.keys(menu).map((category) => (
                  <Dropdown key={category} position="bottomLeft" style={{ width: "240px" }} render={
                    <Dropdown.Menu>
                      {Object.keys(menu[category]).map((item, index) => {
                        if (menu[category][item].children) {
                          return (
                            <Dropdown style={{ width: "120px" }} key={item} position="rightTop" render={
                              <Dropdown.Menu>
                        ...
                        if (menu[category][item].warning) {
                          return (
                            <Popconfirm key={index} title={menu[category][item].warning.title} content={menu[category][item].warning.message} onConfirm={menu[category][item].function} position="right" okText={t("confirm")} cancelText={t("cancel")}>
                              <Dropdown.Item>{t(item)}</Dropdown.Item>
                            </Popconfirm>
                          );
                        }
                        return (
                          <Dropdown.Item key={index} onClick={menu[category][item].function} style={menu[category][item].shortcut && { display: "flex", justifyContent: "space-between", alignItems: "center" }}                            >
                            <div className="w-full flex items-center justify-between">
                              <div>{t(item)}</div>
                              <div className="flex items-center gap-1">
                                {menu[category][item].shortcut && (<div className="text-gray-400"> {menu[category][item].shortcut}</div>)}
  ...
  return <>
    ...
    <Modal modal={modal} exportData={exportData} setExportData={setExportData} title={title} setTitle={setTitle} setPrevTitle={setPrevTitle} setDiagramId={setDiagramId} setModal={setModal} prevTitle={prevTitle} />
